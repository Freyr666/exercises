...
let cents_of : int -> int -> int -> int -> int = fun q d n p ->
  p + (5 * n) + (10 * d) + (25 * q)
...
let rec vars_of (e:exp) : string list =
  ...
  | Const _ -> []
  | Var x -> [x]
  | Mult (e1, e2) | Add (e1, e2) ->
     union (vars_of e1) (vars_of e2)
  | Neg e -> vars_of e
...
let rec lookup (x:string) (c:ctxt) : int32 =
  match c with
  | [] -> raise Not_found
  | (s,v)::tl -> if x = s then v else lookup x tl
...
let rec optimize (e:exp) : exp =
  ...
  | Add (e1,e2) -> begin
      let el,er = (optimize e1, optimize e2) in
      match el,er with
      | (Const il,Const ir) -> Const (Int32.add il ir)
      | (Const 0l,e) | (e,Const 0l) -> e
      | _ -> Add (el, er)
    end
  | Mult (e1,e2) -> begin
      let el,er = (optimize e1, optimize e2) in
      match el,er with
      | (Const il,Const ir) -> Const (Int32.mul il ir)
      | (Const 0l,e) | (e,Const 0l) -> Const 0l
      | (Const 1l,e) | (e,Const 1l) -> e
      | _ -> Add (el, er)
    end
