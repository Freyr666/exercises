...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(items) => {
        def loop(items: List[Json], acc: List[A]): Option[List[A]] =
          items match {
            case Nil => Some(acc.reverse)
            case h::tl => decoder.decode(h) match {
              case None => None
              case Some(x) => loop(tl, x::acc)
            }
          }
        loop(items, Nil)
      }
