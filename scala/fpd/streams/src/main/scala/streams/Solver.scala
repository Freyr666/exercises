...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    def loop(list: List[(Block, Move)]): LazyList[(Block, List[Move])] =
      if (list.isEmpty) LazyList.empty
      else {
        val (b, move) = list.head
        (b, move::history) #:: loop(list.tail)
      }
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
    val neighbors = initial.flatMap({ case (b, hist) =>
      newNeighborsOnly(neighborsWithHistory(b, hist), explored)
    })
    val exp = neighbors.foldLeft(explored)((acc, x) => acc + x._1)
    initial #::: from(neighbors, exp)
  }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from((startBlock, List()) #:: LazyList.empty,
         Set(startBlock))
