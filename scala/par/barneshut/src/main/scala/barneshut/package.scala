...
package object barneshut {
  ...
  extends Quad {
    val (mass, massX, massY) =
      bodies.foldLeft((0 : Float,centerX,centerY)){ case ((m,mx,my),b) =>
        val mass = m + b.mass
        val centX = (mx * m + b.mass * b.x) / mass
        val centY = (my * m + b.mass * b.y) / mass
        (mass, centX, centY)
      }
    ...
    def insert(b: Body): Quad =
      if (size <= minimumSize) Leaf(centerX, centerY, size, bodies :+ b)
        ...
        val ds = size / 4
        val nw = Empty(centerX - ds, centerY - ds, size / 2)
        val ne = Empty(centerX + ds, centerY - ds, size / 2)
        val sw = Empty(centerX - ds, centerY + ds, size / 2)
        val se = Empty(centerX + ds, centerY + ds, size / 2)
        bodies.foldLeft(Fork(nw,ne,sw,se).insert(b))((tree, b) => tree.insert(b))
      }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface {
    ...
    private def clamp(v: Float, min: Float, max: Float) =
      if (v < min) min
      else if (v > max) max
      else v

    def +=(b: Body): SectorMatrix = {
      val xSize = boundaries.width / sectorPrecision
      val ySize = boundaries.height / sectorPrecision
      val xPos = clamp((b.x - boundaries.minX), boundaries.minX, boundaries.maxX)
      val yPos = clamp((b.y - boundaries.minY), boundaries.minY, boundaries.maxY)
      val x = (xPos / xSize).toInt
      val y = (yPos / ySize).toInt
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      assert(boundaries == that.boundaries)
      assert(sectorPrecision == that.sectorPrecision)
