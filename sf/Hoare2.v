...
Proof.
  intros m.
  assert (Bexp : forall st, bassn (BLe (ANum 2) (AId X)) st -> 2 <= (st X)).
  { intros st Hb. unfold bassn in Hb. simpl in Hb.
    destruct (st X).
    + inversion Hb.
    + induction n.
      * inversion Hb.
      * omega. }
  assert (Bnexp : forall st, not (bassn (BLe (ANum 2) (AId X)) st) -> not (2 <= (st X))).
  { intros st Hb. unfold not. intros.
    unfold not in Hb. apply Hb. unfold bassn. simpl.
    destruct (st X).
    + inversion H.
    + induction n.
  ...
  - intros st [Hp Hass].
    apply Bnexp in Hass.
    apply parity_lt_2 in Hass.
    rewrite <- Hass. apply Hp.
  - intros st H.
