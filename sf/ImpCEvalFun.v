...
Example peven_n_1 :
  test_ceval (t_update empty_state X 6) peven
  = Some (0, 0, 0).
Proof. reflexivity. Qed.

Example peven_n_2 :
  test_ceval (t_update empty_state X 3) peven
  = Some (0, 0, 1).
Proof. reflexivity. Qed.
...
Proof.
  intros c st st' Hce.
  induction Hce.
  - exists (S O). reflexivity.
  - exists (S O). simpl. rewrite H. reflexivity.
  - inversion IHHce1 as [ i1 E1 ].
    inversion IHHce2 as [ i2 E2 ].
    exists (S (i1 + i2)). simpl.
    destruct (ceval_step st c1 (i1 + i2)) eqn:Heq.
    + (* ok *)
      (* left statement *)
      assert (IIc1 : i1 <= i1 + i2) by omega.
      apply (ceval_step_more _ _ st st' c1) in IIc1.
      rewrite IIc1 in Heq. inversion Heq.
      rewrite <- H0.
      (* right statement *)
      assert (IIc2 : i2 <= i1 + i2) by omega.
      apply (ceval_step_more _ _ st' st'' c2) in IIc2.
      apply IIc2.
      (* ceval_step_more cleanup *)
      apply E2. apply E1.
    + (* None *)
      simpl. assert (IIc1 : i1 <= i1 + i2) by omega.
      apply (ceval_step_more _ _ st st' c1) in IIc1.
      rewrite IIc1 in Heq. inversion Heq.
      apply E1.
  - inversion IHHce as [ i E ].
    exists (S i). simpl. destruct (beval st b) eqn:Hb.
    + apply E.
    + inversion H.
  - inversion IHHce as [ i E ].
    exists (S i). simpl. destruct (beval st b) eqn:Hb.
  ...
  - inversion IHHce1 as [ i1 E1 ].
    ...
    + assert (IIc1 : i1 <= i1 + i2) by omega.
      apply (ceval_step_more _ _ st st' c) in IIc1.
      rewrite IIc1.
      assert (IIc2 : i2 <= i1 + i2) by omega.
      apply (ceval_step_more _ _ st' st'' (WHILE b DO c END)) in IIc2.
      apply IIc2.
      (* cleanup *)
      apply E2. apply E1.
    + inversion H.
